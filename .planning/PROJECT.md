...
- [ ] A user can create and inspect Cells as life-domain nodes on a hex Flowgrid around a Core.
- [ ] A new Cell works immediately with starter Modules: Generator, Charge Core, Output, and Bloom.
- [ ] The Generator Module is always easy to use and starts an active focus session with minimal friction.
- [ ] Real focus time generates Current, Cell XP, Momentum, milestone progress, and visible flow feedback.
- [ ] Cell milestone completion fires Bloom, creates signal, increases Momentum, and activates the Cell until local day reset.
- [ ] Current can move through a Cell, route toward the Core, and be allocated by the Core between Energy conversion and Core Charge storage.
- [ ] Rejuvenation logs can process Core Charge into Integration progress and grant Module Tokens at thresholds.
...
- [ ] Simulation logic is deterministic, pure TypeScript, and separate from rendering, persistence, and UI concerns.
- [ ] Durable app state is local-first, IndexedDB-backed, migration-aware, and structured as records rather than one giant opaque blob.
...
| Make the Core a dual-output system | Convert/store allocation creates a meaningful strategy tradeoff between immediate Energy and future Integration. | - Pending |
| Make rejuvenation part of the economy | Recovery becomes mechanically meaningful through Core Charge processing and Module Token rewards. | - Pending |
